    ...
    _wordpress_api_fetch__WEBPACK_IMPORTED_MODULE_0___default()({
      ...
      let response = data.map(yearData => {
        let filteredTournaments = yearData.tournaments.filter(tournament => {
          return tournament.config.format === "Individual" && tournament.config.age_group === null && tournament.config.time_control != "Std";
        });
      ...
      function initializeDataTable(time_control) {
        ...
        $("#myTable").DataTable({
          ...
          lengthChange: false,
          responsive: true,
          data: response.filter(tournament => tournament.config.time_control === time_control),
          columns: [{
            data: "year"
          }, {
            data: null,
            ...
            render: function (data, type, row) {
              ...
              if (row.winners) {
                if (row.ltu_winners) {
                  winnersList += '<li class="extra-text">Tarp lietuvių:</li>';
