      ...
      _wordpress_api_fetch__WEBPACK_IMPORTED_MODULE_0___default()({
        ...
        let response = data.map(yearData => {
          let filteredTournaments = yearData.tournaments.filter(tournament => {
            return tournament.config.format === "Individual" && tournament.config.time_control === "Std" && tournament.config.age_group === age_group && (gender === "Boys" && (tournament.config.gender == null || tournament.config.gender === "Open") || gender === "Girls" && tournament.config.gender === "Women");
          });
          ...
          columns: [{
            ...
            render: function (data, type, row) {
              ...
              winners.forEach((winner, index) => {
                ...
                winnersList += `<li class="${className}">${winner.place}. ${winner.name}${winner.rating ? ` (${winner.rating})` : ''}, ${winner.city}</li>`;
              });
              winnersList += "</ul>";
              return winnersList;
            }
            ...
            render: function (data, type, row) {
              ...
              if (row.download_link) {
                links += `<a class="underline" href="${row.download_link}" target="_blank">Atsisiųsti</a><br>`;
              }
              return links;
            }
          }]
