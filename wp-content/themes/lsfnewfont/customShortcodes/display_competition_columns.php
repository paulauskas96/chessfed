...
// cia yra trys stulpeliai varzybu ivyko, vyksta, bus
...
function display_competitions_columns($atts)
{
  ...
  foreach ($soon_to_begin as $post) {
    ...
    $output .= '<div class="image-wrapper" style="min-width: 60px; min-height: 60px; text-align: center;">';
    if (has_post_thumbnail($post->ID)) {
      $output .= '<img class="featured-image" style="max-width: 60px; max-height: 60px;" src="' . get_the_post_thumbnail_url($post->ID) . '" alt="Featured Image">';
    }
    $output .= '</div>';
  ...
  $output .= '<div class="column">';
  $output .= '<h4 class="column-title">Įvyko</h4>';
  $output .= '<div class="competition-list-wrapper">';
